...
#class DigitKerasModel(DigitKerasCallbacks):#, DigitKerasEvaluate):
    ...
    def load_model(self, filename=None, compile=False, verbose=True)->object:
        ...
        if os.path.isfile(filename):
            if verbose: print("Model will be retrieved from %s compile = %d"%(filename,compile))
            ...
            if verbose: print("Model %s was retrieved from %s "%(self.modelname, filename))
            return self.model
        else:
    ...
    def eval_save_predictions(self, epoch= 9, eval_gen=None, y_pred=None, y_prob=None, max_save=32, verbose=True):
        ...
        # ----------------------------
        # print("Eval_save_predictions for %s"%eval_gen.name)
        ...
        # save predictions
        # ----------------
        print("Saving %d predictions to %s"%(len(y_files), self.pred_dir))
        ...
        # Summary predictions
        # -------------------
        print("Summarize %d predictions "%len(y_files))
        ...
        # ------------------------
        print("Saving %d probabilities to %s"%(len(y_files), self.prob_dir))
        ...
        # ---------------------
        print("Summarize %d probabilites "%len(y_files))
        self.eval_summary_prob(imgs, names, epoch)    
        

        # save masks
        # ----------
        print("Saving %d segmentations to %s"%(max_save, self.seg_dir))
        ...
        print("Summarize %d segmentations "%len(y_files))
        ...
        # save_img
        # --------
        print("Saving %d images to %s"%(max_save, self.pred_img_dir))
        imgs=[]; names=[]
        ...
        # Summary images
        # --------------
        print("Summarize %d images "%len(y_files))
    ...
    def eval_predict_model(self, eval_gen=None, ind=None, verbose=True):
        ...
        print("Load weights skipped")
        #self.load_weights(verbose=True)
        
        if eval_gen is None:
            print("eval_predict_model eval_gen set to val_gen")
            eval_gen = self.val_gen
        else:
            print("eval_predict_model eval_gen taken over")
        ...
        try:
            ...
            print("Start prediction Datagen %s for %d slices"%(eval_gen.name, cnt))
        ...
        print("Model predicted test_gen for %d items"%(y_pred.shape[0]))
        ...
        self.eval_summary_conf(self.cnf_matrix, epoch=9, normalize=False) 
        
        print("On_epoch_end : Eval_summary confusion_matrix_normalized") #epoch end
        self.eval_summary_conf(self.cnf_matrix, epoch=9, normalize=True) 
        self.eval_summary_curves(self.y_test, self.y_pred, self.y_prob, epoch=9,  verbose=True)
        # self.eval_summary_pcl(epoch=9)
        # self.eval_summary_metrics(self.y_test, self.y_pred, epoch=9,  verbose=True)
        return y_pred
    ...
    def eval_predict_step(self, val_gen=None, verbose=False):
        ...
        # 
        try:
            print("Eval_predict step Datagen %s, batch_size  %d "%(val_gen.name, val_gen.batch_size))
            print("Eval_predict step Datagen %s  items       %d "%(val_gen.name, len(val_gen)))
            tf.autograph.set_verbosity(0)
            y_pred= self.model.predict(val_gen, batch_size = val_gen.batch_size,verbose=0)
            print(y_pred.shape)
            
            if y_pred is not None:
                pass
                print("Eval_predict step Datagen  y_pred.shape %s "%str(y_pred.shape))
                print("Eval_predict step Datagen  y_pred.shape %s "%str(y_pred.dtype))
            else:    
                print("Eval_predict step Datagen  Error y_pred is None")
                return None
                
        except BaseException as e:
            print("eval_predict_step error %s"%val_gen.name)
            print("%s"%str(e))
    ...
    def eval_save_conf_matrix(self, epoch, cnf_matrix, normalize=False, verbose=True):
        ...
        if verbose: print("Save conf_matrix %d %s"%(epoch, line))
